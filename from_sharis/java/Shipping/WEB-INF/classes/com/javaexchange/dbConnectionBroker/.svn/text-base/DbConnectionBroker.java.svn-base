    ...
    public void destroy(int millis) throws SQLException 
    {
        ...
        for(int i=0; i < currConnections; i++) {
            try {
                ...
                if(debugLevel > 0) {
                    dbLog.println("ERROR - Cannot close connections on Destroy");
                }
            }
        }
        if(useCount > 0) {
            //bt-test successful
            String msg="ERROR - SHUTDOWN UNSAFE: Had to close "+useCount+
            ...
            dbLog.println("SHUTDOWN SUCCESSFUL");
        // Close all open files
        //dbLog.close();
    }//End destroy()
