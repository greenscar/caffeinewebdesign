...
//A CUSTOM TAG LIBRARY MUST IMPLEMENT ONE OF THE INTERFACES IN:
import javax.servlet.jsp.tagext.*;

public class TagCal extends TagSupport{
    /*
    ...
    //When the tag is encountered in a JSP, the doStartTag method is executed.
    public int doStartTag(){
        //Use the current day as the default date.
        if((mCalendar==null) && (this.month==0) && (this.day==0) && (this.year==0))
            setCalCurrentDate();
        if((this.month!=0) && (this.day!=0) && (this.year!=0)){
            this.month--;
            setCalDate(this.month, this.day, this.year);
        }
        //Set the default output stream to the JspWriter in the pageContext
        if(mOut == null)
            setOut(pageContext.getOut());
        
        try{
            //Print the calendar or print the date as a string
            if(!dateStringOnly)
                printCal();
            else
                mOut.println(getCurrentDate());
        }catch(ServletException e){
            System.out.println("TagCal error: " + e.getMessage());
        }catch(IOException e){
            System.out.println("IOException in TagCal: " + e.getMessage());
        }
        //Ensure there is no body to the process.
        return(SKIP_BODY);
    }
    //Bean Methods
    ...
    public void printCal() throws ServletException{
        if(mOut == null)
