...
session_start();
if((@$_POST["Submit"]) || (session_is_registered("web_employment_admin"))){
	if(session_is_registered("web_employment_admin")){
		//LOGIN IS SUCCESSFUL
		//echo("login was good.");
		$web_employment_admin = $_SESSION['web_employment_admin'];
		Header("Location: index.php");
	}
  	else if((!empty($_POST["login"])) && (!empty($_POST["pwd"])) ){
		$search_string = "SELECT * FROM emp_admins WHERE login = '" . $_POST["login"] . "' AND pwd = '" . $_POST["pwd"] . "'";
		...
		if (($search_occurances != '0')) { 
			...
			session_register("web_employment_admin");
			$web_employment_admin = new A_Person($row["login"]);
			Header("Location: index.php");
		} 
 	   	else {
