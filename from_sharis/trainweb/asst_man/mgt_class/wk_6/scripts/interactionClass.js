...
function MM_interaction(theSelf,
  ...
  this.browserIsOldNS = ((navigator.appName.indexOf('Netscape') != -1) && (navigator.appVersion<"5"));
...
function MM_intFindObject(objName,  parentObj) {
  ...
  if (!curObj) {
    ...
    if (parentObj[objName] != null) curObj = parentObj[objName]; //at top level
      ...
      if (!found && NS) {
	if (parentObj.layers && parentObj.layers.length > 0) {
		...
		if (Math.floor(navigator.appVersion.charAt(0))>=5) {
