...
function isntNewNS() {
	return (!((navigator.appName=="Netscape") && (navigator.appVersion>="5")))
}
...
function MM_dragInit() {
  with (this) {
    ...
    if (_obj) {
     ...
     if (isntNewNS()) {
      _width =  parseInt((_parent.browserIsOldNS)?_obj.clip.width:_obj.offsetWidth);
      _height = parseInt((_parent.browserIsOldNS)?_obj.clip.height:_obj.offsetHeight);
     } else {
	_width = parseInt(_obj.style.width);
	_height = parseInt(_obj.style.height);
     }
      getPosition();
...
function MM_dragUpdate(dragging) {
  ...
  with (this) {
    if (dragging) {
      if (this.onDrag != null) {
        x = parseInt((_parent.browserIsOldNS)?_obj.pageX:parseInt(_obj.style.left));
        y = parseInt((_parent.browserIsOldNS)?_obj.pageY:parseInt(_obj.style.top));
...
function MM_dragGetPosition() {
  if (this._obj) with (this) {
    ...
    _posX= (_parent.browserIsOldNS)?_obj.pageX:parseInt(_obj.style.left);
    _posY= (_parent.browserIsOldNS)?_obj.pageY:parseInt(_obj.style.top);
  }
}
  ...
  with (this) {
    if (document.allLayers != null) {
      for (i=0; i<document.allLayers.length; i++) {
        ...
        aLayerZ = (_parent.browserIsOldNS)?aLayer.zIndex:aLayer.style.zIndex;
        if (aLayerZ > maxZ) maxZ = aLayerZ;
      }
      eval('_obj.'+((_parent.browserIsOldNS)?'':'style.')+'zIndex=maxZ+1');
  } }
}
